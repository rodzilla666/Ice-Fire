...
public:
    ...
    void update(HDC hdc, Player* boy, Player* girl){
        ...
        // check if pushed
        if((boy->x_pos<=x_pos+width&& boy->x_pos>x_pos+width-15 && boy->y_pos+boy->height-5<=y_pos+height && boy->y_pos>y_pos-5)){
            x_pos--;
        }
        if((boy->x_pos+boy->width>=x_pos && boy->x_pos+boy->width<x_pos+15 && boy->y_pos+boy->height-5<=y_pos+height && boy->y_pos>y_pos-5)){
            x_pos++;
        }
        if((girl->x_pos<=x_pos+width&& girl->x_pos>x_pos+width-15 && girl->y_pos+girl->height-5<=y_pos+height && girl->y_pos>y_pos-5)){
            x_pos--;
        }
        if((girl->x_pos+girl->width>=x_pos && girl->x_pos+girl->width<x_pos+15 && girl->y_pos+girl->height-5<=y_pos+height && girl->y_pos>y_pos-5)){
            x_pos++;
        }
    }
};
